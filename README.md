...
But at some point, I got frustrated with reveal.js too. This text is not intended as a critic of reveal.js since it is, as I said, an incredible too, but let me list a few caveats I encountered while using reveal.js. First, it was sometime hard to tinker with the CSS sometimes, because the tool being so polished, its base CSS was intefering too much with my mental model of the webpage, preventing me to achieve exactly what I needed. At some point, I managed to ditch most of what I did not need and managed to design a style I liked and which I had control on. 

Later, I hit a more annoying limitation. On a slide, I had a fragment block that I wanted to show on the second transition and then on the fourth one, but not on the third, which seems like a pretty common scenario when writing slides. Beamer allows it via the `\onslide` function, but reveal.js does not, and it is apparently [not planning on changing it](https://github.com/hakimel/reveal.js/issues/889). While I am perfectly fine with hakimel not wanting to implement this functionality, I started getting annoyed by this and had other cool ideas to improve the fragment feature. Another annoyance was that it is not straightforward to display a [fragment right on the beginning of a slide](https://github.com/hakimel/reveal.js/issues/2560), which is also something I often found myself to want. Finally, the PDF export was not always working and I often could not figure out why. 
